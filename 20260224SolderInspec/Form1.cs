...
namespace _20260224SolderInspec
{
    public class Form1 : Form
    {
        ...
        private void InitializeSettingsTab(TabPage tab)
        {
            ...
            btnSave.Click += (s, e) => { SaveConfig(); MessageBox.Show("保存しました。"); }; tab.Controls.Add(btnSave);
        }
        ...
        private void InitializeInspectionTab(TabPage tab)
        {
            ...
            tab.Controls.Add(new Label { Text = "--- 検査モード 選択 (複数ONで並列実行) ---", Location = new Point(10, y), AutoSize = true, ForeColor = Color.Magenta, Font = new Font(this.Font, FontStyle.Bold) }); y += 22;
                ...
                case STATE_STABILIZING:
                    if (_appSettings.TriggerMode == "Plc")
                    {
                        ...
                        if ((DateTime.Now - _stabilityStartTime).TotalMilliseconds > targetDelay)
                        {
                            ...
                            // ★ハイブリッド救済フォールバック（外形のみONで運用している場合の救済措置）
        ...
        private void LoadConfig()
        {
            ...
            try
            {
                ...
                {
                    bool useOuter = bool.Parse(uoe);
                    _measurement.EnableOuterTiltCheck = useOuter;
                    _measurement.EnableHoleCheck = !useOuter;
                }
                else
                {
